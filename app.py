...
DEFAULT_REBALANCE_MONTHS = 1        # rebalance frequency (months)

@st.cache_data(show_spinner=False)
def load_returns(path: str):
    """
    Load a CSV with columns date + return_rank_1…return_rank_n
    Strips “%”, converts to float (e.g. 5.34% → 5.34), and sets date index.
    Returns the DataFrame and a sorted list of return‐column names.
    """
    df = pd.read_csv(path)
...
def compute_rank_weights(n: int, cash_pct: float) -> np.ndarray:
    ...
    E.g. if n=3 and cash_pct=0.10, weights = [3,2,1]/(3+2+1) * 0.90.
    """
    if n < 1:
        return np.array([])
    budget = 1.0 - cash_pct
...
def main():
    ...
    # ── 1) Load the CSV of rank‐based returns ────────────────────
    df_ret, ret_cols = load_returns("ranked_returns_top15.csv")

    # ── 2) Build the “Actual” series from your hand‐entered monthly returns
    monthly_returns = [
    ...
    actual_series = (
        pd.Series(monthly_returns, index=df_ret.index[:len(monthly_returns)])
          .add(1)                             # convert % → multiplier
          .cumprod()                         # compound
          .mul(INITIAL_INVESTMENT)           # scale to $100 start
          .rename("Actual Value")
    )

    # ── 3) Sidebar: pick how many positions + cash split ──────────
    ...
    # display rank weights
    weights = compute_rank_weights(n, cash_pct)
    st.sidebar.markdown("**Rank Weights**")
    for i, w in enumerate(weights, start=1):
        st.sidebar.write(f"Rank {i}: {w:.2%}")
    st.sidebar.write(f"Cash: {cash_pct:.2%}")

    # ── 4) Build a rolling weight matrix, rebalance monthly ───────
    periods = len(df_ret)
    rebalance_flag = (np.arange(periods) % DEFAULT_REBALANCE_MONTHS == 0)
    w = pd.DataFrame(0.0, index=df_ret.index, columns=ret_cols)
    # on each rebalance row, assign top‐n weights
    w.loc[rebalance_flag, ret_cols[:n]] = weights
    w = w.ffill().fillna(0.0)  # carry forward between rebalances

    # ── 5) Compute portfolio return each period (in decimals) ────
    # note: df_ret[ret_cols] are floats like 5.34 meaning 5.34%, so divide by 100
    period_ret = (w * df_ret[ret_cols] / 100.0).sum(axis=1)

    # ── 6) Compound that series starting from $100 ────────────────
    model_series = (
        (1 + period_ret)
          .cumprod()
    ...
    # tiny zoom on y‐axis
    yrange = combined.max().max() - combined.min().min()
    ax.set_ylim(combined.min().min() - 0.02*yrange,
                combined.max().max() + 0.02*yrange)
    st.pyplot(fig)
